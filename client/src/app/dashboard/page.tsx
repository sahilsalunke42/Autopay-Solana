  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(231,195,91,0.12),_transparent_26%),linear-gradient(180deg,#0d0d10_0%,#050505_100%)] text-white">
      <div className="mx-auto max-w-[1300px] px-5 py-6 md:px-8 md:py-8 xl:px-10">
        ...
        <section className="grid gap-6 py-3 lg:grid-cols-[1.05fr_0.95fr] lg:gap-7 lg:py-6">
          <motion.div
          ...
          >
            <div>
              ...
              <h1 className="mt-2.5 max-w-3xl font-sans text-4xl font-semibold leading-[1.08]">
                Welcome back
              </h1>
            ...
            <Card className="relative overflow-hidden p-6 md:p-7">
              ...
              <div className="relative">
                <p className="text-xs uppercase tracking-[0.2em] text-gold-200/80">Wallet Security</p>
                <h2 className="mt-2 font-sans text-2xl font-semibold">Link your private key</h2>
                <p className="mt-1.5 text-sm text-white/65">Your private key is encrypted and only used for transaction execution.</p>
          ...
          >
            <Card className="p-6">
              <div className="flex items-center gap-2 text-gold-300">
                ...
                <span className="text-xs uppercase tracking-[0.18em]">Create task</span>
              </div>
              <h3 className="mt-3 font-sans text-2xl font-semibold">New autopay</h3>
              <div className="mt-5 space-y-3">
                <div>
                  <textarea
                    ...
                    className="w-full rounded-lg border border-white/20 bg-white/5 px-3.5 py-2.5 text-sm text-white placeholder-white/40 focus:border-gold-300/50 focus:outline-none"
                    rows={4}
                  />
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between">
                <h3 className="font-sans text-xl font-semibold">Tasks</h3>
                <span className="text-xs uppercase tracking-[0.18em] text-white/45">{tasks.length} items</span>
              </div>
                  ...
                  tasks.map((task) => (
                    <div key={task.id} className="rounded-2xl border border-white/10 bg-black/20 p-4">
                      <div className="mb-3 flex items-start justify-between gap-3">
                        <div>
                          <p className="text-base font-medium text-white">{task.amount} {task.token}</p>
                          <p className="mt-0.5 text-xs text-white/50">
                            {task.frequency} · <span className={task.status === "ACTIVE" ? "text-green-400" : "text-yellow-400"}>{task.status}</span>
                          </p>
                          <p className="mt-0.5 truncate text-xs text-white/45">{task.receiverAddress}</p>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {task.status === "ACTIVE" && (
                          <>
                            <Button onClick={() => void handleExecuteTask(task.id)} className="h-8 px-3 text-xs">
                              Execute
                            </Button>
                            <Button onClick={() => void handlePauseTask(task.id)} className="h-8 px-3 text-xs border-white/30 bg-transparent text-white hover:bg-white/5">
                              <Pause className="h-3 w-3 mr-1" /> Pause
                            </Button>
                          </>
                        )}
                        {task.status === "PAUSED" && (
                          <Button onClick={() => void handleResumeTask(task.id)} className="h-8 px-3 text-xs">
                            <Play className="h-3 w-3 mr-1" /> Resume
                          </Button>
                        )}
                        <Button onClick={() => void handleDeleteTask(task.id)} className="h-8 px-3 text-xs border-red-500/30 bg-transparent text-red-400 hover:bg-red-500/10">
